...
// AdminAccessService needs IDbContextFactory<AppDbContext> (registered below) - registration
// order doesn't matter here, DI resolves lazily at first use, not at Add* call time.
builder.Services.AddScoped<IAdminAccessService, AdminAccessService>();
// Scoped, not the more usual Singleton for an IAuthorizationHandler - it depends on
// IAdminAccessService, itself Scoped (via IDbContextFactory); a Singleton handler capturing a
// Scoped dependency through constructor injection is a captive-dependency bug, not just a style
// choice, so this has to follow IAdminAccessService's own lifetime.
...
builder.Services.AddAuthorization(options =>
{
    ...
    // A custom requirement/handler, not policy.RequireAssertion - that callback is synchronous,
    // and AdminOnlyAuthorizationHandler needs to query the database (AdminUser rows), not just
    // check the appsettings.json AdminEmails list. See IAdminAccessService's own doc comment for
    // the two-tier design this backs.
