...
static async Task MigrateLlmDefinitionsAsync(SqlConnection source, AppDbContext target, bool dryRun)
{
    ...
    while (await reader.ReadAsync())
    {
        ...
        {
            Id = id,
            // V1 only ever called AWS Bedrock (confirmed via the WSO2 proxy sequence).
            Provider = LlmProvider.AwsBedrock,
            Identifier = reader.GetString(1),
            Name = reader.GetString(2),
            Description = reader.GetString(3),
            MaxTokens = reader.IsDBNull(4) ? null : reader.GetInt32(4),
            DefaultAdherence = reader.GetDecimal(5),
            DefaultCreativity = reader.GetDecimal(6),
...
static async Task MigrateHelperDefinitionsAsync(SqlConnection source, AppDbContext target, bool dryRun)
{
    ...
    while (await reader.ReadAsync())
    {
        ...
        target.HelperDefinitions.Add(new HelperDefinition
        {
            ...
            Creativity = reader.IsDBNull(4) ? null : reader.GetDecimal(4),
            Adherence = reader.IsDBNull(5) ? null : reader.GetDecimal(5),
            CreativityAdjustmentAllowance = reader.IsDBNull(6) ? null : reader.GetDecimal(6),
            AdherenceAdjustmentAllowance = reader.IsDBNull(7) ? null : reader.GetDecimal(7),
